...
plateNormalizationServer <- function(id, stored_plates_data, selected_studyexpplate) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$normalize, {
      ...
      # Load plate data
      norm_plate_data <- load_plate_data(
     ...
     # plate_data <- load_plate_data(stored_plates_data = stored_plates_data, normset_str = normset_str, selected_antigen = selected_antigen, selected_feature = selected_feature)
      ...
      plate_group <- aggregate(x = norm_plate_data$raw_value,
                               by = list(norm_plate_data$study_accession,norm_plate_data$experiment_accession,norm_plate_data$dilution,norm_plate_data$antigen,norm_plate_data$feature),
                               FUN = mean
      )
      names(plate_group)[names(plate_group) == "x"] <- "normal_reference"

      norm_plate_data <- merge(norm_plate_data,
                          plate_group,
      ...
      #sorts data
      norm_plate_data <- norm_plate_data[order(norm_plate_data$plateid,norm_plate_data$normal_reference),]

      # get fitted models
      model_results <- fit_models(norm_plate_data)
      ...
      plots <- plot_plate_fits(norm_plate_data = norm_plate_data, fitted_models = fitted_models)
