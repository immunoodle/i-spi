...
observeEvent(input$run_mcmc_calc, ignoreInit = TRUE, {
  ...
  if (get_status(df, scope, "interpolated") != "completed") {
    showNotification(
      "Interpolated concentrations must be completed first.",
  ...
  tryCatch({
    ...
    best_glance <- filter_glance_scope(
      best_glance, scope, experiment, plate
    )
    ...
    # ── Fetch all MCMC input data once ───────────
    
    id_set <- best_glance$best_glance_all_id
    
    combined_df <- fetch_combined_mcmc(
      study,
      proj,
      id_set,
      conn
    ...
    for (i in seq_along(id_set)) {
      ...
      curve_df <- combined_df[
        combined_df$best_glance_all_id == id,
      ]
      ...
      update_mcmc_progress(i, n_total, row)
      
      message("Running JAGS for ID: ", id)
      
      res <- run_jags_predicted_concentration(
        glance_row   = row,
      ...
      if (!is.null(res)) {
        ...
        if (!"mcmc_set" %in% names(res)) {
          res$mcmc_set <- curve_df$mcmc_set[
            match(res$row_id, curve_df$row_id)
          ]
        }
    ...
    if (is.null(results_df) || nrow(results_df) == 0) {
      showNotification("MCMC produced no results.", type = "error")
      return()
    }
    
    result_pred_all   <- results_df[results_df$mcmc_set == "pred_se", ]
    result_sample_all <- results_df[results_df$mcmc_set == "sample_se", ]
    result_sample_all$final_robust_concentration <- result_sample_all$dilution * result_sample_all$raw_robust_concentration
    ...
    showNotification(
      paste0("MCMC Robust completed for ", scope_label, "."),
      type = "message",
      duration = 10
    )
  ...
  error = function(e) {
    ...
    showNotification(
      paste0("MCMC Robust error: ", conditionMessage(e)),
      type = "error",
      duration = 15
    )
...
#     id_set      <- best_glance$best_glance_all_id
#     best_glance$last_concentration_calc_method[best_glance$best_glance_all_id %in% id_set] <- "pending"
#     
#     # upsert_best_curve(
#     #   conn   = conn,
#     #   df     = best_glance[best_glance$best_glance_all_id %in% id_set, ],
#     #   schema = "madi_results",
#     #   table  = "best_glance_all",
#     #   notify = shiny_notify(session)
#     # )
...
#       message("Running PREDICTION GRID for ID: ", my_id)
#       
...
#       message("Running SAMPLE DATA for ID: ", my_id)
...
#     best_glance$last_concentration_calc_method[best_glance$best_glance_all_id %in% id_set] <- "completed"
#     
#     best_glance_complete <<- best_glance
#     
#     ## ── 5. Save / update DB ──
...
#     # )
#     # 
#     # ## ── 7. Upsert pred and sample results ──
#     # upsert_best_curve(
#     #   conn   = conn,
#     #   df     = result_pred_all,
#     #   schema = "madi_results",
#     #   table  = "result_pred_all",
#     #   notify = shiny_notify(session)
#     # )
#     # 
#     # upsert_best_curve(
#     #   conn   = conn,
#     #   df     = result_sample_all,
#     #   schema = "madi_results",
#     #   table  = "result_sample_se_all",
#     #   notify = shiny_notify(session)
#     # )
#     
#     showNotification(
